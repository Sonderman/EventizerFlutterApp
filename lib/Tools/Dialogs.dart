...
Future<bool> askingDialog(BuildContext context, String title, Color backgroundColor) async {
  return await showDialog(
    context: context,
    builder: (dcontext) {
      return AlertDialog(
        ...
        actions: <Widget>[
          TextButton(
            ...
            child: const Text("Hayır", style: TextStyle(fontSize: 16.0, color: Colors.white)),
          ),
          TextButton(
            onPressed: () {
              Navigator.pop(dcontext, true);
            },
            child: const Text("Evet", style: TextStyle(fontSize: 16.0, color: Colors.white)),
          ),
        ],
      );
    },
                  ...
                  userService.sendFeedback(controller.text).then((value) {
                    if (value) {
                      ...
                      Fluttertoast.showToast(
                        msg: "Geri Bildiriminiz Gönderilmiştir Teşekkür Ederiz.",
